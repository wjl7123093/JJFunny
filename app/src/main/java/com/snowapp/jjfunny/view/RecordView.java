...
public class RecordView extends View implements View.OnLongClickListener, View.OnClickListener {
    ...
    private static final int PROGRESS_INTERVAL = 100;   // 间隔 100ms 更新一次
    private final Paint fillPaint;      // 录制按钮画笔
    private final Paint progressPaint;  // 进度条画笔
    private int progressMaxValue;       // 最大进度
    private final int radius;           // 录制按钮圆角
    private final int progressWidth;    // 进度条宽度（半径）
    private final int progressColor;    // 进度条颜色
    private final int fillColor;        // 录制按钮背景色
    private final int maxDuration;      // 最大时长
    private int progressValue;          // 当前进度
    private boolean isRecording;        // 是否开启录制
    private long startRecordTime;       // 开始录制时间
    private onRecordListener mListener; // 录制监听器
